...
df$sd_f0 <- ifelse(df2$trial_number %in% c(2:10), NA, df2$sd_f0)
...
just_variability_model <- glm(Diagnosis ~ sd_f0, data = df2, na.action = "na.omit")
...
# function 2 has not been updated to work in cross-validation
roc_and_confmat2 <- function(model){
  # first line of confusion matrix
  pred_column <- logit2prob(predict(model))
  # roc curve
  rocCurve <- roc(response = df$Diagnosis2, predictor = pred_column, na.rm = TRUE)
  areaUnderCurve <- auc(rocCurve)
  aucConfidenceInterval <- ci(rocCurve)
  # now back to confusion matrix
  pred_column[pred_column >0.5] = "Schizophrenia"
  pred_column[pred_column <=0.5] = "Control"
  pred_column <- as.factor(pred_column)
  Diagnosis <- as.factor(df$Diagnosis2)
  confMatrix <- confusionMatrix(data = pred_column, reference = df$Diagnosis2, positive = "Schizophrenia")
  # collect what we want to return
  ResultList <- list(c(confMatrix$overall['Accuracy'], confMatrix$byClass['Sensitivity'], confMatrix$byClass['Specificity'], confMatrix$byClass['Pos Pred Value'], confMatrix$byClass['Neg Pred Value']), areaUnderCurve)
  return(ResultList)
}
...
# roc_and_confmat2(just_variability_model) # doesn't work because of different lenghts of pred_column and df$Diagnosis
...
for (fold in folds){
  ...
  model = glmer(scz ~ speechrate..nsyll.dur. + (1 | ID), data = train, family = "binomial", glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
  trainRMSE[i] <- RMSE(train$scz, train$prediction)
  testRMSE[i] <- RMSE(test$scz, test$prediction)
  i <- i + 1
  print(roc_and_confmat(test$prediction, test$Diagnosis))
}

trainRMSE
testRMSE

crossValidation <- data.frame(trainRMSE, testRMSE)
crossValidation$diff <- abs(crossValidation$trainRMSE - crossValidation$testRMSE)
mean(crossValidation$diff)
```

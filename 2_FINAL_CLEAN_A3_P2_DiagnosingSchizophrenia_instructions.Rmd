...
# And also making a Diagnosis column with values for only the rows with sd_f0_2 values
#df$Diagnosis2 <- df$Diagnosis
#df$Diagnosis2 <- ifelse(df$trial_number %in% c(2:10), NA, df$Diagnosis2)


# We need to ungroup afterewards, because the code (confusion matrix and roc != 1) works if i don't run these lines above
df <- ungroup(df)

# And making a separate dataframe for the pitch variability and new diagnosis variables (where we remove all of the rows with NA)
df2 <- df
df2 <- na.omit(df2)
...
just_variability_model <- glm(Diagnosis ~ sd_f0, data = df2, family = "binomial") # This model does not have repeated measures, so we are using the dataframe with only one pitch variability observation per participant.
...
roc_and_confmat <- function(model){
...
df2$pred_variability <- logit2prob(predict(just_variability_model))
rocCurve_var <- roc(response = df2$Diagnosis, predictor = df2$pred_variability)
###

roc_and_confmat2 <- function(model){
  # first line of confusion matrix
  pred_column <- logit2prob(predict(model))
  # roc curve
  rocCurve <- roc(response = df2$Diagnosis, predictor = pred_column)
  areaUnderCurve <- auc(rocCurve)
  aucConfidenceInterval <- ci(rocCurve)
  # now back to confusion matrix
  pred_column[pred_column >0.5] = "Schizophrenia"
  pred_column[pred_column <=0.5] = "Control"
  pred_column <- as.factor(pred_column)
  Diagnosis <- as.factor(df2$Diagnosis)
  confMatrix <- confusionMatrix(data = pred_column, reference = df2$Diagnosis, positive = "Schizophrenia")
  # collect what we want to return
  ResultList <- list(c(confMatrix$overall['Accuracy'], confMatrix$byClass['Sensitivity'], confMatrix$byClass['Specificity'], confMatrix$byClass['Pos Pred Value'], confMatrix$byClass['Neg Pred Value']), areaUnderCurve)
  return(ResultList)
}
...
roc_and_confmat(just_pausedur_model)
roc_and_confmat2(just_variability_model) # doesn't work because of different lengths of pred_column and df$Diagnosis  ##### Edit: I think I fixed it
roc_and_confmat(just_proportion_model)
roc_and_confmat(just_speechrate_model)
```
...
roc_and_confmat_3 <- function(prediction){
  # first line of confusion matrix
  pred_column <- logit2prob(predict(model))
  # roc curve
  rocCurve <- roc(response = df$Diagnosis, predictor = pred_column)
...
for (fold in folds){
  ...
  model = glmer(Diagnosis ~ speechrate..nsyll.dur. + (1 | ID), data = train, family = "binomial")
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
  trainRMSE_m6[i] <- RMSE(train$Diagnosis, fitted(model))
  testRMSE_m6[i] <- RMSE(test$Diagnosis, test$prediction)
  i <- i + 1
  # print(roc_and_confmat(model))
}
